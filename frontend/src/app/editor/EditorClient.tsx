...
export default function EditorClient({ jobId }: { jobId: string }) {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 1024) { // Mobile/Tablet threshold
        const screenWidth = window.innerWidth;
        const availableWidth = screenWidth - 40; // 20px padding each side
        const baseWidth = 850; // A4 approx width
    ...
    <AuthGuard>
      <div className="print-path flex flex-col md:flex-row h-[calc(100vh-64px)] w-full bg-black industrial-grid selection:bg-cyan-accent selection:text-black font-sans relative overflow-x-hidden">
        ...
        <div className={`print-path print:p-0 flex-grow h-full bg-[#111111] print:bg-white flex flex-col overflow-hidden relative ${isSidebarHidden ? 'w-full' : 'w-auto'}`}>
          ...
          {/* PERSISTENT CONTROL DOCK: Unified Command Center */}
          <div className="no-print h-autp min-h-[64px] bg-zinc-950/60 backdrop-blur-md border-b border-zinc-900 flex flex-wrap items-center justify-between px-4 sm:px-6 py-2 flex-shrink-0 z-50 gap-y-3">

            {/* LEFT: NAVIGATION & TIMELINE */}
            <div className="flex items-center gap-2 sm:gap-4">
              <button
                onClick={() => setIsSidebarHidden(!isSidebarHidden)}
                className="p-2 sm:p-2.5 bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white rounded-sm flex items-center gap-2 transition-all hover:bg-zinc-800 active:scale-95 shadow-lg"
                title={isSidebarHidden ? "Open Design Studio" : "Full Focus View"}
              >
                <LucideTerminal className={`w-3.5 h-3.5 ${isSidebarHidden ? 'text-zinc-500' : 'text-cyan-accent'}`} />
                <span className="text-[9px] uppercase tracking-[0.2em] font-bold hidden lg:block">
                  {isSidebarHidden ? 'Studio' : 'Focus'}
                </span>
              </button>

              {resumeData && (
                <div className="flex items-center gap-0.5 bg-zinc-900 border border-zinc-800 p-0.5 rounded-sm">
                  <button
                    onClick={stack.undo}
                    disabled={!stack.canUndo}
                    className={`p-2 sm:p-2.5 transition-all rounded-sm group/undo ${stack.canUndo ? 'text-cyan-accent hover:bg-zinc-800' : 'text-zinc-800 cursor-not-allowed opacity-30'}`}
                    title="Switch to Previous Version"
                  >
                    <LucideUndo2 className={`w-3.5 h-3.5 ${stack.canUndo ? 'group-hover:scale-110 active:scale-90' : ''} transition-transform`} />
                  </button>
                  <div className="px-2 sm:px-4 border-x border-zinc-800 flex flex-col items-center justify-center min-w-[80px] sm:min-w-[120px]">
                    <span className="text-[10px] sm:text-[11px] font-black text-white uppercase tracking-widest whitespace-nowrap">
                      Version {resumeData.version_number || stack.currentIndex}
                    </span>
                    <span className="text-[7px] font-mono text-zinc-500 tracking-tighter uppercase hidden sm:block">
                      Timeline: {stack.currentIndex} of {stack.totalDepth}
                    </span>
                  </div>
                  <button
                    onClick={stack.redo}
                    disabled={!stack.canRedo}
                    className={`p-2 sm:p-2.5 transition-all rounded-sm group/redo ${stack.canRedo ? 'text-cyan-accent hover:bg-zinc-800' : 'text-zinc-800 cursor-not-allowed opacity-30'}`}
                    title="Switch to Next Version"
                  >
                    <LucideRedo2 className={`w-3.5 h-3.5 ${stack.canRedo ? 'group-hover:scale-110 active:scale-90' : ''} transition-transform`} />
                  </button>
                </div>
              )}
            </div>

            {/* CENTER: STUDIO ZOOM (New Home) */}
            <div className="flex items-center gap-0.5 bg-zinc-900 border border-zinc-800 p-0.5 rounded-sm overflow-hidden order-last sm:order-none mx-auto sm:mx-0">
              <button
                onClick={() => setZoomLevel(Math.max(0.4, zoomLevel - 0.1))}
                className="p-2 sm:p-2.5 hover:bg-zinc-800 text-zinc-500 hover:text-white transition-colors"
                title="Zoom Out"
              >
                <LucideZoomOut className="w-3.5 h-3.5" />
              </button>
              <div className="px-2 sm:px-3 text-[9px] font-mono font-bold text-zinc-500 min-w-[45px] text-center border-x border-zinc-800">
              ...
              <button
                ...
                title="Zoom In"
              >
                <LucideZoomIn className="w-3.5 h-3.5" />
              </button>
              <button
                onClick={() => {
                  const screenWidth = window.innerWidth;
                  const availableWidth = screenWidth - (isSidebarHidden ? 80 : 480);
                  setZoomLevel(Math.min(1, Math.max(0.4, availableWidth / 850)));
                }}
                className="ml-0.5 p-2 sm:p-2.5 bg-cyan-accent/5 hover:bg-cyan-accent text-cyan-accent hover:text-black transition-all"
                title="Fit to Screen"
              >
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="hidden md:flex items-center gap-2 pr-4 border-r border-zinc-800">
                ...
                <span className="text-[8px] font-black uppercase tracking-widest text-emerald-500 whitespace-nowrap">
                  Artifact Valid
                </span>
              </div>
              ...
              {status === 'success' && resumeData && (
                <Button
                  className="bg-emerald-600 hover:bg-emerald-500 text-white rounded-none h-10 px-4 sm:px-6 text-[10px] uppercase tracking-[0.2em] font-black transition-all flex items-center gap-2 group"
                  onClick={handlePrint}
                >
          ...
          <div className="flex-grow overflow-y-auto custom-scrollbar flex flex-col items-center p-4 sm:p-6 md:p-10 lg:p-16 xl:p-20 relative bg-zinc-950/40">
            ...
            <div
              className="relative transition-all duration-300 ease-out flex-shrink-0 origin-top print:!transform-none print:!m-0 print:!p-0"
              style={{
                transform: `scale(${zoomLevel})`,
                marginBottom: `-${(1 - zoomLevel) * 1150}px` // Negative margin to reclaim vertical space after scaling
              }}
            >
              <div
                id="printable-resume-mount"
                className={`print:shadow-none print:border-none a4-paper bg-white shadow-[0_30px_90px_rgba(0,0,0,0.15)] rounded-sm border border-zinc-300 p-[10mm] relative transition-all duration-700 print:!m-0 ${pageSize === 'LETTER' ? 'w-full max-w-[816px] min-h-[1056px] size-letter' : 'w-full max-w-[850px] min-h-[1100px] size-a4'}`}
              >
